...
void CSocket::printTDInfo() {
    ...
    if ((currtime - m_lastprintTime) > 10) {
        ...
        hps_log_stderr(
            0, "连接池中空闲连接/总连接/要释放的连接(%d/%d/%d)。", m_freeconnectionList.size(), m_connectionList.size(),
            m_recyconnectionList.size());
        hps_log_stderr(0, "当前时间队列大小(%d)。", m_timerQueuemap.size());
        hps_log_stderr(
            0, "当前收消息队列/发消息队列大小分别为(%d/%d)，丢弃的待发送数据包数量为%d。", tmprmqc, tmpsmqc,
...
int CSocket::hps_epoll_oper_event(
    int fd,             // socket
    ...
    if (epoll_ctl(m_epollhandle, eventtype, fd, &ev) == -1) {
        hps_log_stderr(
            errno, "CSocket::hps_epoll_oper_event()中epoll_ctl(%d,%ud,%ud,%d)失败.", fd, eventtype, flag, bcaction);
        return -1;
    }
    return 1;
}
...
int CSocket::hps_epoll_process_events(int timer) {
    ...
    */
    ...
    if (events == 0) {
        ...
        // 一直阻塞的情况下返回0，发生错误
        hps_log_error_core(
            HPS_LOG_ALERT, 0, "CSocket::hps_epoll_process_events()中epoll_wait()没超时却没返回任何事件!");
        return 0;
    }
    ...
    for (int i = 0; i < events; ++i) {
        ...
        //   */
        //   hps_log_error_core(HPS_LOG_DEBUG, 0, "CSocket::hps_epoll_process_events()中遇到了fd=-1的过期事件:%p.",
        //   p_Conn); continue;
        // }
        ...
        //   hps_log_error_core(HPS_LOG_DEBUG, 0,
        //   "CSocket::hps_epoll_process_events()中遇到了instance值改变的过期事件:%p.",
        //                      p_Conn);
        //   continue;
        // }
