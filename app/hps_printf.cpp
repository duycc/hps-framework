 ...
 * @return u_char*
 */
static u_char *hps_sprintf_num(u_char *buf, u_char *last, uint64_t ui64, u_char zero, uintptr_t hexadecimal,
                               uintptr_t width);
    ...
    if (*fmt == '%') //%开头的一般都是需要被可变参数 取代的
    {
      ...
      zero = (u_char)((*++fmt == '0')
                          ...
                          : ' '); //判断%后边接的是否是个'0',如果是zero = '0'，否则zero = '
                                  //'，一般比如你想显示10位，而实际数字7位，前头填充三个字符，就是这里的zero用于填充
      ...
      for (;;) //一些特殊的格式，我们做一些特殊的标记【给一些变量特殊值等等】
      {
        ...
        case '.': //其后边必须跟个数字，必须与%f配合使用，形如
                  //%.10f：表示转换浮点数时小数部分的位数，比如%.10f表示转换浮点数时，小数点后必须保证10位数字，不足10位则用0来填补；
      ...
      case 'f': //一般 用于显示double类型数据，如果要显示小数部分，则要形如 %.5f
        ...
        if (frac_width) //如果是%d.2f，那么frac_width就会是这里的2
        {
          ...
          frac = (uint64_t)((f - (double)ui64) * scale +
                            0.5); //取得保留的那些小数位数，【比如  %.2f
                                  //，对应的参数是12.537，取得的就是小数点后的2位四舍五入，也就是54】
                                  //如果是"%.6f", 21.378，那么这里frac = 378000

          if (frac == scale) //进位，比如    %.2f ，对应的参数是12.999，那么  = (uint64_t) (0.999 * 100 + 0.5)  =
                             //(uint64_t) (99.9 + 0.5) = (uint64_t) (100.4) = 100 而此时scale == 100，两者正好相等
          {
            ui64++;   //正整数部分进位
            frac = 0; //小数部分归0
          }
        ...
        //正整数部分，先显示出来
        buf = hps_sprintf_num(buf, last, ui64, zero, 0, width); //把一个数字 比如“1234567”弄到buffer中显示
        ...
        if (frac_width) //指定了显示多少位小数
        {
          ...
          buf = hps_sprintf_num(buf, last, frac, '0', 0,
...
static u_char *hps_sprintf_num(u_char *buf, u_char *last, uint64_t ui64, u_char zero, uintptr_t hexadecimal,
                               uintptr_t width) {
  u_char *p, temp[HPS_INT64_LEN + 1]; // sizeof("-9223372036854775808") - 1 = 20
  size_t len;
  uint32_t ui32;

  static u_char hex[] = "0123456789abcdef"; // %xd格式符相关，显示的16进制数中 a-f 小写
  static u_char HEX[] = "0123456789ABCDEF"; // %Xd格式符相关，显示的16进制数中 A-F 大写
  ...
  if (hexadecimal == 0) { // 非16进制显示
    if (ui64 <=
        (uint64_t)
            HPS_MAX_UINT32_VALUE) // HPS_MAX_UINT32_VALUE :最大的32位无符号数：十进制是‭4294967295‬
    {
      ...
      1) //如果显示一个十六进制数字，格式符为：%xd，则这个条件成立，要以16进制数字形式显示出来这个十进制数,a-f小写
  {
    ...
    } while (ui64 >>= 4); // ui64 >>= 4     --->   ui64 = ui64 >> 4 ,而ui64 >>
                          // 4是啥，实际上就是右移4位，就是除以16,因为右移4位就等于移动了1111；
                          //相当于把该16进制数的最末尾一位干掉，原来是 12 D687, >> 4后是 12
                          // D68，如此反复，最终肯定有=0时导致while不成立退出循环 比如 1234567 / 16 = 77160(0x12D68)
  ...
  len = (temp + HPS_INT64_LEN) - p; //得到这个数字的宽度，比如 “7654321”这个数字 ,len = 7
  ...
  {
    *buf++ =
        zero; //填充0进去到buffer中（往末尾增加），比如你用格式
              // ngx_log_stderr(0, "invalid option: %10d\n", 21);
              //显示的结果是：nginx: invalid option:         21  ---21前面有8个空格，这8个弄个，就是在这里添加进去的；
  }

  len = (temp + HPS_INT64_LEN) - p; //还原这个len，也就是要显示的数字的实际宽度【因为上边这个while循环改变了len的值】
  ...
  return hps_memcpy(buf, p, len); //把最新buf返回去；
}
