...
/*
   hps_log_stderr(0, "invalid option: \"%s\"", argv[0]);  //nginx: invalid option: "./nginx"
   hps_log_stderr(0, "invalid option: %10d", 21);         //nginx: invalid option:         21  ---21前面有8个空格
   hps_log_stderr(0, "invalid option: %.6f", 21.378);     //nginx: invalid option: 21.378000
   ---%.这种只跟f配合有效，往末尾填充0 hps_log_stderr(0, "invalid option: %.6f", 12.999);     //nginx: invalid
   option: 12.999000 hps_log_stderr(0, "invalid option: %.2f", 12.999);     //nginx: invalid option: 13.00
   hps_log_stderr(0, "invalid option: %xd", 1678);        //nginx: invalid option: 68E
   hps_log_stderr(0, "invalid option: %Xd", 1678);        //nginx: invalid option: 68E
   hps_log_stderr(15, "invalid option: %s , %d", "testInfo",326);        //nginx: invalid option: testInfo , 326
   hps_log_stderr(0, "invalid option: %d", 1678);
   */
void hps_log_stderr(int err, const char *fmt, ...) {
  va_list args; //创建一个va_list类型变量
  u_char
      errstr[HPS_MAX_ERROR_STR + 1]; // 2048  -- ************  +1是我自己填的，感谢官方写法有点小瑕疵，所以动手调整一下
  u_char *p, *last;

  memset(
      errstr, 0,
      sizeof(
          errstr)); //我个人加的，这块有必要加，至少在va_end处理之前有必要，否则字符串没有结束标记不行的；***************************

  last =
      errstr +
      HPS_MAX_ERROR_STR; // last指向整个buffer最后去了【指向最后一个有效位置的后面也就是非有效位】，作为一个标记，防止输出内容超过这么长,
                         //其实我认为这有问题，所以我才在上边errstr[HPS_MAX_ERROR_STR+1]; 给加了1
  //比如你定义 char tmp[2]; 你如果last = tmp+2，那么last实际指向了tmp[2]，而tmp[2]在使用中是无效的

  p = hps_memcpy(errstr, "nginx: ", 7); // p指向"nginx: "之后
  ...
  *p++ = '\n'; //增加个换行符
  ...
  if (hps_log.fd > STDERR_FILENO) //如果这是个有效的日志文件，本条件肯定成立，此时也才有意义将这个信息写到日志文件
  {
...
// hps_log_error_core(5,8,"这个XXX工作的有问题,显示的结果是=%s","YYYY");
void hps_log_error_core(int level, int err, const char *fmt, ...) {
  u_char *last;
  u_char errstr[HPS_MAX_ERROR_STR + 1]; //这个+1也是我放入进来的，本函数可以参考ngx_log_stderr()函数的写法；
  ...
  hps_slprintf(strcurrtime,
               (u_char *)-1, //若用一个u_char *接一个 (u_char *)-1,则 得到的结果是 0xffffffff....，这个值足够大
               "%4d/%02d/%02d %02d:%02d:%02d", //格式是 年/月/日 时:分:秒
               tm.tm_year, tm.tm_mon, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
  p = hps_memcpy(errstr, strcurrtime,
                 strlen((const char *)strcurrtime)); //日期增加进来，得到形如：     2019/01/08 20:26:07
  p = hps_slprintf(p, last, " [%s] ", err_levels[level]); //日志级别增加进来，得到形如：  2019/01/08 20:26:07 [crit]
  p = hps_slprintf(p, last,
                   "%P: ", hps_pid); //支持%P格式，进程id增加进来，得到形如：   2019/01/08 20:50:15 [crit] 2037:

  va_start(args, fmt);                   //使args指向起始的参数
  p = hps_vslprintf(p, last, fmt, args); //把fmt和args参数弄进去，组合出来这个字符串
  va_end(args);                          //释放args
  ...
  *p++ = '\n'; //增加个换行符
  ...
  while (1) {
    ...
    //写日志文件
    n = write(hps_log.fd, errstr, p - errstr); //文件写入成功后，如果中途
    if (n == -1) {
      //写失败有问题
      if (errno == ENOSPC) //写失败，且原因是磁盘没空间了
      {
        ...
        if (hps_log.fd != STDERR_FILENO) //当前是定位到文件的，则条件成立
        {
...
void hps_log_init() {
  ...
  if (plogname == NULL) {
    //没读到，就要给个缺省的路径文件名了
    plogname = (u_char *)HPS_ERROR_LOG_PATH; //"logs/error.log" ,logs目录需要提前建立出来
  }
  hps_log.log_level =
      p_config->GetIntDefault("LogLevel", HPS_LOG_NOTICE); //缺省日志等级为6【注意】 ，如果读失败，就给缺省日志等级
  // nlen = strlen((const char *)plogname);

  //只写打开|追加到末尾|文件不存在则创建【这个需要跟第三参数指定文件访问权限】
  // mode = 0644：文件访问权限， 6: 110    , 4: 100：     【用户：读写， 用户所在组：读，其他：读】
  // 老师在第三章第一节介绍过 hps_log.fd = open((const char *)plogname,O_WRONLY|O_APPEND|O_CREAT|O_DIRECT,0644);
  // //绕过内和缓冲区，write()成功则写磁盘必然成功，但效率可能会比较低；
  hps_log.fd = open((const char *)plogname, O_WRONLY | O_APPEND | O_CREAT, 0644);
  if (hps_log.fd == -1) //如果有错误，则直接定位到 标准错误上去
  {
    hps_log_stderr(errno, "[alert] could not open error log file: open() \"%s\" failed", plogname);
    hps_log.fd = STDERR_FILENO; //直接定位到标准错误去了
  }
  return;
}
