//===--------------------------- app/hps_log.cpp - HP-Server --------------------------------------------*- C++ -*-===//
// brief :
//   Print log and write log file.
//
// author: YongDu
// date  : 2021-09-12
//===--------------------------------------------------------------------------------------------------------------===//
...
void hps_log_stderr(int err, const char* fmt, ...) {
    ...
    va_list args;
    char errstr[HPS_MAX_ERROR_STR + 1];
    char *p, *last;
...
void hps_log_init() {
    ...
    if (NULL == plogname) {
        plogname = (char*)HPS_ERROR_LOG_PATH; // 读取失败，使用默认路径
    }
